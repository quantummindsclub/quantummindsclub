...
def send_participant_emails(event_id):
    ...
    try:
        ...
        for p in participants:
            if not p.email:
                continue
            achievement_url = f"https://neuralwired.onrender.com/events/{event_id}+{p.college_code}+{p.student_id}"
...
def download_certificate():
    ...
    try:
        ...
        qr_data = f"https://neuralwired.onrender.com/events/{data['event_id']}+{data['college_code']}+{data['student_id']}"
        qr = qrcode.QRCode(
            version=1,
